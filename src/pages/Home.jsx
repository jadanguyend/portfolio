...
function MinifigPopup({ minifig, onClose }) {
  ...
  return createPortal(
    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 flex items-center justify-between bg-grayLight-900 dark:bg-grayDark-900 text-grayLight-50 dark:text-grayDark-50 px-6 py-4 rounded-[16px] shadow-lg"
...
export default function Home() {
  ...
  const heroMinis = [
    { id: 1, src: miniMe, x: "38%", y: "36%", r: 0, name: "The Original", series: "Powered by caffeine" },
    { id: 2, src: miniMagnify, x: "12%", y: "10%", r: -5, name: "The Analyst", series: "Precision-driven and observant" },
    { id: 3, src: miniDog, x: "75%", y: "80%", r: 0, name: "The Wanderer", series: "Moves with curiosity and independence" },
    { id: 4, src: miniSkateboard, x: "7%", y: "75%", r: -5, name: "The Playful", series: "Bold, playful, and always moving forward" },
    { id: 5, src: miniRead, x: "84%", y: "15%", r: 6, name: "The Archivist", series: "Gathers knowledge and stories" },
  ];

  /* ✅ NEW: stagger visibility */
  /* ✅ FIXED: guaranteed replay on reload */
  const [visibleMinis, setVisibleMinis] = useState([]);
  const [startAnimation, setStartAnimation] = useState(false);

  useEffect(() => {
    // ensures this runs AFTER React's double-mount in dev
...
useEffect(() => {
  ...
  setVisibleMinis([]); // reset

  heroMinis.forEach((mini, index) => {
    const t = setTimeout(() => {
      setVisibleMinis((prev) => [...prev, mini.id]);
    }, index * 250);
    ...
    <Layout footer={<Footer />}>
      ...
      >
        ...
        <div className="absolute inset-0 z-[20]">
          {heroMinis
            .filter((item) => visibleMinis.includes(item.id))
            .map((item) => (
              <SelectableMinifig
                key={item.id}
                item={item}
